...
public:
	...
	floatType			observed_volume; //外部观测流量
	...
	int                 db_ltype;  // 数据库中的原始“道路类型/连杆类型”等分类（不参与成本函数选择），例如 10 表示质心连杆
...
public:
	...
	inline int		   id_() {return dest->id; } //return dest's id
...
public:
	...
	map<int, int>        centroidNodeToZone; // 质心连杆节点 id -> 交通小区 area_id（构网时填充，ReportPG 写库用）
...
/*==============================================================================================
  ...
  =============================================================================================*/
	...
	int					BuildPostgreSQL(const char* dbConnStr, bool loadod, const TNM_LINKTYPE ltype, const string& networkTableName, const string& odTableName);
...
public:
	...
	TNM_SPATH*           SPath(TNM_SNODE* origin, TNM_SNODE* dest);
	TNM_SPATH*           GetSPath_R_(TNM_SNODE* origin, TNM_SNODE* dest);
	void                 UpdateSPR(TNM_SNODE* rootDest);
	void                 UpdateSPR(TNM_SNODE* origin, TNM_SNODE* dest);
	...
	void                 InitialSubNet4(int sptreeLen = 0, bool createpath = false);
...
public:
	...
	void         SPTreeD(TNM_SNODE *);
	void         SPTreeD(TNM_SNODE* origin, TNM_SNODE* dest);
	virtual bool InsertANode(TNM_SNODE *) {return false;}
	virtual      TNM_SNODE* GetNextNodeMin() { return NULL; }
protected:
