                                                         ...
                                                         double rgp)
{
    ...
    payload["summary.stage"] = "tool_estimation";
...
int run_tool_estimation(const std::map<std::string, std::string>& req,
                         std::string& message,
                         std::map<std::string, std::string>& payload,
                         const std::string& progress_file)
{
    ...
    if (project_id <= 0 || user_id <= 0) {
        message = "参数错误：project_id、user_id 须为正整数；case_id 为方案编号。";
        return -1;
    }

    const bool scheme_observation = peek_scheme_observation_prefix(param2, req);
    if (scheme_observation && case_id < 0) {
        message = "参数错误：方案前缀 OD 反推（other_observation）须 case_id >= 0。";
        return -1;
    }
    ...
    const bool use_scheme_prefix = resolve_use_scheme_prefix(param2, req);
    ...
    try {
        ...
        OD_ESTIMATION odes;
        apply_default_parameters(odes);
        const bool tool_observation = peek_tool_observation_relaxed_defaults(param2, req);
        if ((scheme_observation || tool_observation) && !peek_use_roadway_observed(param2)) {
            apply_scheme_observation_relaxed_defaults(odes);
        }
        ...
        int build_ret = odes.Build("grpc_od_estimation", "", NETPOSTGRESQL);
        if (build_ret != 0) {
...
int main(int argc, char** argv)
{
    ...
    if (request_file.empty() || response_file.empty()) {
        std::cerr << "usage: tool_estimation_cli --request-file <path> --response-file <path> [--progress-file <path>]" << std::endl;
        return 2;
    }
    ...
    std::string message;
    std::map<std::string, std::string> payload;
    int code = run_tool_estimation(req, message, payload, progress_file);
