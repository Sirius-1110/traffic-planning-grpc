...
if(!TNM_ParseDbIOConfig(inFileName, outFileName, dbConfig, dbErrorMessage))
{
...
if(dbConfig.enabled)
{
if(dbConfig.iterResultTable.empty()) dbConfig.iterResultTable = "log_result";
if(dbConfig.linkResultTable.empty()) dbConfig.linkResultTable = "link_result";
if(dbConfig.pathResultTable.empty()) dbConfig.pathResultTable = "path_result";
        TNM_PGDB db;
...
auto formatNumber = [](double value) -> string
{
        ostringstream oss;
        oss.setf(ios::fixed);
...
auto buildPathString = [](TNM_SPATH *path) -> string
            {
                string text;
                for(int a = static_cast<int>(path->path.size()) - 1; a >= 0; a--)
                {
                char res[64];
                sprintf(res, "%d", path->path[a]->id);
                text += res;
        if(a > 0) text += "_";
        }
        return text;
};
if(!db.Execute("BEGIN", dbErrorMessage))
{
                    cout<<dbErrorMessage<<endl;
        return 1;
            }
        ...
        {
                string iterTable;
                if(!TNM_ResolveTableName(dbConfig.iterResultTable, dbConfig.defaultSchema, iterTable, dbErrorMessage))
                    {
                            cout<<dbErrorMessage<<endl;
                            return 1;
                    }
                    if(!db.Execute("DROP TABLE IF EXISTS " + iterTable, dbErrorMessage) || !db.Execute("CREATE TABLE " + iterTable + " (iter integer, conv double precision, cpu_time double precision)", dbErrorMessage))
                    {
                            cout<<dbErrorMessage<<endl;
                            return 1;
                    }
                    for (vector<ITERELEM*>::iterator pv = iterRecord.begin();pv != iterRecord.end(); pv++)
                    {
                            string sql = "INSERT INTO " + iterTable + " (iter, conv, cpu_time) VALUES (" + to_string((*pv)->iter, std::dec) + "," + formatNumber((*pv)->conv) + "," + formatNumber((*pv)->time) + ")";
                            if(!db.Execute(sql, dbErrorMessage))
                            {
                                    cout<<dbErrorMessage<<endl;
                                    return 1;
                            }
            ...
            {
                    string linkTable;
                    if(!TNM_ResolveTableName(dbConfig.linkResultTable, dbConfig.defaultSchema, linkTable, dbErrorMessage))
                    {
                            cout<<dbErrorMessage<<endl;
                            return 1;
                    }
                    if(!db.Execute("DROP TABLE IF EXISTS " + linkTable, dbErrorMessage) || !db.Execute("CREATE TABLE " + linkTable + " (from_node integer, to_node integer, mode text, flow double precision, v_over_c double precision)", dbErrorMessage))
                    {
                            cout<<dbErrorMessage<<endl;
                            return 1;
                    }
                    for (int i = 0;i<network->numOfLink;i++)
                    {
                            ...
                            double vc = (link->capacity > 1e-10 ? link->volume / link->capacity : 0.0);
                            if(network->IsMultiClass)
                            {
                                    for(int k=0;k<network->numOfClass;k++)
                                    {
                                            string sql = "INSERT INTO " + linkTable + " (from_node, to_node, mode, flow, v_over_c) VALUES (" + to_string(link->tail->id, std::dec) + "," + to_string(link->head->id, std::dec) + "," + TNM_QuotePgLiteral(network->multiclassInfo[k]->type) + "," + formatNumber(link->mc_volume[k]) + "," + formatNumber(vc) + ")";
                                            if(!db.Execute(sql, dbErrorMessage))
                                            {
                                                    cout<<dbErrorMessage<<endl;
                                                    return 1;
                            ...
                            {
                                    string sql = "INSERT INTO " + linkTable + " (from_node, to_node, mode, flow, v_over_c) VALUES (" + to_string(link->tail->id, std::dec) + "," + to_string(link->head->id, std::dec) + ",'all'," + formatNumber(link->volume) + "," + formatNumber(vc) + ")";
                                    if(!db.Execute(sql, dbErrorMessage))
                                    {
                                            cout<<dbErrorMessage<<endl;
                                            return 1;
            ...
            {
                    string pathTable;
                    if(!TNM_ResolveTableName(dbConfig.pathResultTable, dbConfig.defaultSchema, pathTable, dbErrorMessage))
                    {
                            cout<<dbErrorMessage<<endl;
                            return 1;
                    }
                    if(!db.Execute("DROP TABLE IF EXISTS " + pathTable, dbErrorMessage) || !db.Execute("CREATE TABLE " + pathTable + " (from_node integer, to_node integer, path text, mode text, flow double precision, num_links integer)", dbErrorMessage))
                    {
                            cout<<dbErrorMessage<<endl;
                            return 1;
                    }
                            ...
                            for(int j=0;j<orgn->numOfDest;j++)
                            {
                                    ...
                                    if(network->IsMultiClass)
                                    {
                                            for(int k=0;k<dest->mc_pathset.size();k++)
                                            {
                                                    for(int h=0;h<dest->mc_pathset[k]->pathSet.size();h++)
                                                    {
                                                            ...
                                                            string sql = "INSERT INTO " + pathTable + " (from_node, to_node, path, mode, flow, num_links) VALUES (" + to_string(orgn->origin->id, std::dec) + "," + to_string(dest->dest->id, std::dec) + "," + TNM_QuotePgLiteral(buildPathString(path)) + "," + TNM_QuotePgLiteral(network->multiclassInfo[k]->type) + "," + formatNumber(path->flow) + "," + to_string(static_cast<int>(path->path.size()), std::dec) + ")";
                                                            if(!db.Execute(sql, dbErrorMessage))
                                                            {
                                                                    cout<<dbErrorMessage<<endl;
                                                                    return 1;
                                    ...
                                    else
                                    {
                                            for(int h=0;h<dest->pathSet.size();h++)
                                            {
                                                    ...
                                                    string sql = "INSERT INTO " + pathTable + " (from_node, to_node, path, mode, flow, num_links) VALUES (" + to_string(orgn->origin->id, std::dec) + "," + to_string(dest->dest->id, std::dec) + "," + TNM_QuotePgLiteral(buildPathString(path)) + ",'all'," + formatNumber(path->flow) + "," + to_string(static_cast<int>(path->path.size()), std::dec) + ")";
                                                    if(!db.Execute(sql, dbErrorMessage))
                                                    {
                                                            cout<<dbErrorMessage<<endl;
                                                            return 1;
            ...
            if(!db.Execute("COMMIT", dbErrorMessage))
            {
                    cout<<dbErrorMessage<<endl;
                    return 1;
            }
    ...
    if(reportIterHistory)
    {
            string iteFileName = inFileName + "log.csv";
