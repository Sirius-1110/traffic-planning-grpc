...
namespace
{
	bool TNM_ParseDbInt(const string &value, int &target)
	{
		return TNM_FromString<int>(target, value, std::dec);
	}

	bool TNM_ParseDbFloat(const string &value, floatType &target)
	{
		return TNM_FromString<floatType>(target, value, std::dec);
	}

	int TNM_FlushDbOrigin(TNM_SNET *network, int originId, const vector<int> &destinations, const vector<floatType> &demands)
	{
		if(originId < 0 || destinations.empty()) return 0;
		TNM_SNODE *onode = network->CatchNodePtr(originId, true);
		if(onode == NULL)
		{
			cout<<"\n\tnode "<<originId<<" is not a valid node object. "<<endl;
			return 1;
		}
		TNM_SORIGIN* pOrg = network->CreateSOriginP(onode, destinations.size());
		if(pOrg == NULL)
		{
			cout<<"cannot create static origin object "<<originId<<"!"<<endl;
			return 1;
		}
		pOrg->m_tdmd = 0.0;
		for(int j = 0; j < destinations.size(); j++)
		{
			TNM_SNODE *dnode = network->CatchNodePtr(destinations[j], true);
			if(dnode == NULL)
			{
				cout<<"\n\tnode "<<destinations[j]<<" is not a valid node object. "<<endl;
				continue;
			}
			pOrg->SetDest(j + 1, dnode, demands[j]);
			pOrg->m_tdmd += demands[j];
		}
		return 0;
	}

	int TNM_BuildSZFormatASIIDB(TNM_SNET *network, const TNM_DB_IO_CONFIG &dbConfig)
	{
		string errorMessage;
		string nodeTable;
		string linkTable;
		string demandTable;
		if(!TNM_ResolveTableName(dbConfig.nodeTable, dbConfig.defaultSchema, nodeTable, errorMessage))
		{
			cout<<errorMessage<<endl;
			return 1;
		}
		if(!TNM_ResolveTableName(dbConfig.linkTable, dbConfig.defaultSchema, linkTable, errorMessage))
		{
			cout<<errorMessage<<endl;
			return 1;
		}
		if(!TNM_ResolveTableName(dbConfig.demandTable, dbConfig.defaultSchema, demandTable, errorMessage))
		{
			cout<<errorMessage<<endl;
			return 1;
		}
		TNM_PGDB db;
		if(!db.Open(dbConfig.conninfo, errorMessage))
		{
			cout<<"Cannot connect to PostgreSQL: "<<errorMessage<<endl;
			return 1;
		}
		NODE_VALCONTAINER nval;
		LINK_VALCONTAINER lval;
		vector<vector<string> > rows;
		string sql = "SELECT node_id, is_centroid FROM " + nodeTable + " ORDER BY node_id";
		if(!db.Query(sql, rows, errorMessage))
		{
			cout<<"Failed to query node table: "<<errorMessage<<endl;
			return 1;
		}
		string line;
		network->centroidSet.clear();
		cout<<"read node table..."<<endl;
		for(int rowIndex = 0; rowIndex < rows.size(); rowIndex++)
		{
			if(rows[rowIndex].size() < 2) continue;
			int nodeid = 0;
			int isCentroid = 0;
			if(!TNM_ParseDbInt(rows[rowIndex][0], nodeid) || !TNM_ParseDbInt(rows[rowIndex][1], isCentroid))
			{
				cout<<"Invalid node row in PostgreSQL input table."<<endl;
				return 1;
			}
			if((nodeid - network->nodeVector.size()) > 1)
			{
				for(int i = network->nodeVector.size() + 1; i < nodeid; i++)
				{
					nval.type = BASND;
					nval.id = i;
					nval.dummy = false;
					network->CreateNewNode(nval);
				}
			}
			nval.type = BASND;
			nval.id = nodeid;
			nval.dummy = false;
			TNM_SNODE* newnode = network->CreateNewNode(nval);
			if(newnode == NULL)
			{
			...
			if(isCentroid != 0)
			{
				...
				network->centroidSet.push_back(newnode);
			}
			else
			{
		...
		rows.clear();
		sql = "SELECT from_node, to_node, fft, capacity, toll, alpha1, alpha2, link_type FROM " + linkTable + " ORDER BY from_node, to_node";
		if(!db.Query(sql, rows, errorMessage))
		{
			cout<<"Failed to query link table: "<<errorMessage<<endl;
			return 1;
		}
		cout<<"read link table..."<<endl;
		int lid = 0;
		for(int rowIndex = 0; rowIndex < rows.size(); rowIndex++)
		{
			if(rows[rowIndex].size() < 8) continue;
			int tail = 0;
			int head = 0;
			floatType fft = 0.0, toll = 0.0, cap = 0.0, alpha1 = 0.0, alpha2 = 0.0;
			if(!TNM_ParseDbInt(rows[rowIndex][0], tail) || !TNM_ParseDbInt(rows[rowIndex][1], head) || !TNM_ParseDbFloat(rows[rowIndex][2], fft) || !TNM_ParseDbFloat(rows[rowIndex][3], cap) || !TNM_ParseDbFloat(rows[rowIndex][4], toll) || !TNM_ParseDbFloat(rows[rowIndex][5], alpha1) || !TNM_ParseDbFloat(rows[rowIndex][6], alpha2))
			{
				cout<<"Invalid link row in PostgreSQL input table."<<endl;
				return 1;
			}
			string ty = rows[rowIndex][7];
			if(tail != head)
			{
				lid++;
				if(ty == "bpr")
				...
				lval.id = lid;
				lval.dummy = false;
				lval.tail = network->CatchNodePtr(tail, true);
				lval.head = network->CatchNodePtr(head, true);
				if(cap <= 1e-6)
				{
				...
				if(ty == "bpr")
				{
					lval.par.push_back(alpha1);
					lval.par.push_back(alpha2);
				}
				else
				{
					lval.par.push_back(alpha1);
				...
				lval.capacity = cap;
				TNM_SLINK *link = network->CreateNewLink(lval);
				if(link == NULL)
				{
				...
				link->beckFlow = 0.0;
				if(ty == "alk")
					link->period = network->period;
			}
		}
		rows.clear();
		sql = "SELECT from_node, to_node, demand FROM " + demandTable + " WHERE from_node <> to_node AND demand > 0 ORDER BY from_node, to_node";
		if(!db.Query(sql, rows, errorMessage))
		{
			cout<<"Failed to query demand table: "<<errorMessage<<endl;
			return 1;
		}
		cout<<"read demand table..."<<endl;
		int lastor = -1;
		vector<int> destinations;
		vector<floatType> demands;
		for(int rowIndex = 0; rowIndex < rows.size(); rowIndex++)
		{
			if(rows[rowIndex].size() < 3) continue;
			int originId = 0;
			int destinationId = 0;
			floatType demand = 0.0;
			if(!TNM_ParseDbInt(rows[rowIndex][0], originId) || !TNM_ParseDbInt(rows[rowIndex][1], destinationId) || !TNM_ParseDbFloat(rows[rowIndex][2], demand))
			{
				cout<<"Invalid demand row in PostgreSQL input table."<<endl;
				return 1;
			}
			if(lastor == -1 || lastor == originId)
			{
				destinations.push_back(destinationId);
				demands.push_back(demand);
			}
			else
			{
				if(TNM_FlushDbOrigin(network, lastor, destinations, demands) != 0)
				{
					return 1;
				}
				destinations.clear();
		...
		network->UpdateNodeNum();
		network->UpdateLinkNum();
		network->UpdateOriginNum();
		network->buildStatus = 1;
		return 0;
	}
}
	...
	while(!linkcsv.eof())
	{
		...
		if(words.size()>=9)
		{
			...
			if(tail!=head)
			{
				...
				{
					lval.par.push_back(alpha1);//�̶�����
					lval.par.push_back(alpha2);//�������
					lval.par.push_back(alpha3);//����ͨ������
				}
				else
					cout<<"Error: link "<<tail<<" - "<<head<<"'s type = "<<ty<<endl;
	...
	while(!odcsv.eof())
	{
		...
		if(words.size()>=3)
		{
			...
			if(onode->scanStatus==0)
			{
				...
				pOrg->destInfoList.push_back(df); //��¼�յ㣬֮��һ�𴴽���������destvector�����ݽṹ��
...
int TNM_SNET::BuildSZFormatASII()
{
	...
	if(!TNM_ParseDbIOConfig(networkName, "", dbConfig, dbErrorMessage))
	{
	...
	if(dbConfig.enabled)
	{
		return TNM_BuildSZFormatASIIDB(this, dbConfig);
	}
	//cout<<"BuildSZFormatAS"<<endl;
	NODE_VALCONTAINER nval;
	LINK_VALCONTAINER lval;
...
int TNM_SNET::BFM_KSP_Search(TNM_SNODE *rootOrg, int K)
{
	...
	while (!scanQ.empty())
	{
		...
		iter->second->used = false;//����node�Ĵ�kspPathElem�ڲ���scanQ��
