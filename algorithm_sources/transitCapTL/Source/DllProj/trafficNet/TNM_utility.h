...
struct TNM_EXT_CLASS TNM_DB_IO_CONFIG
{
	TNM_DB_IO_CONFIG();
	bool enabled;
	string conninfo;
	...
	string walkTable;
	string tripTable;
	string linkResultTable;
	string pathResultTable;
	string iterResultTable;
	string summaryResultTable;
};

TNM_EXT_CLASS bool		TNM_IsDbIOSpec(const string &spec);
TNM_EXT_CLASS bool		TNM_ParseDbIOConfig(const string &inSpec, const string &outSpec, TNM_DB_IO_CONFIG &config, string &errorMessage);
TNM_EXT_CLASS bool		TNM_ResolveTableName(const string &tableName, const string &defaultSchema, string &qualifiedName, string &errorMessage);
TNM_EXT_CLASS string	TNM_QuotePgLiteral(const string &value);

class TNM_EXT_CLASS TNM_PGDB
{
public:
	TNM_PGDB();
	~TNM_PGDB();
	bool Open(const string &conninfo, string &errorMessage);
	bool Execute(const string &sql, string &errorMessage);
	bool Query(const string &sql, vector<vector<string> > &rows, string &errorMessage);
	void Close();
	bool IsOpen() const {return m_conn != NULL;}
private:
	void *m_conn;
};
