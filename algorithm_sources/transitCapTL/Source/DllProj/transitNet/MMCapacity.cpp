...
long double ClampNonNegative(long double value)
{
	...
	switch(GetTransitLinkType())
    {
		...
		case PTLink::ABOARD:
			...
			if (m_tlSym==PTLink::ASYMMTRIC) // link cost function is asymmric
			{
				...
				{
				   g_cost = cost + ClampNonNegative(static_cast<long double>(mu + lambda* ( rLink->volume -  rLink->cap)));
				}
			}
			else
			{
		...
		case PTLink::ENROUTE:
			...
			if(m_tlSym==PTLink::ASYMMTRIC) // link cost function is asymmric
			{
				...
				if (m_tlCap == PTLink::TL)
				{
					g_cost = cost + ClampNonNegative(static_cast<long double>(mu + lambda* (volume - cap)));		
				}
				else
				{
				   g_cost = cost;		
...
void	PTLink::TL_UpdateGeneralPTLink_Cost_DerCost(floatType lambda)
{
	...
	switch(GetTransitLinkType())
    {
		...
		case PTLink::ENROUTE:
			...
			fft		=	pars[5];
			cost = alpha_3 * fft + beta_3 * pow ((tvol + (gamma_3 - 1) * rtvol) / cap2, n2);
			g_cost = cost + ClampNonNegative(static_cast<long double>(mu + lambda* (tvol - cap)));
			//derivative cost
			pfdcost  = beta_3 * n2 * pow ((tvol + (gamma_3 - 1) * rtvol) / cap2, n2 - 1) * (1 / cap2);
			rpfdcost = beta_3 * n2 * pow ((tvol + (gamma_3 - 1) * rtvol) / cap2, n2 - 1) * ((gamma_3 - 1) / cap2);
...
void PTNET::IniTransitLinkMultiplier()
{
	...
	for(int i=0;i<numOfLink;i++)
	{
		...
		if (link->GetTransitLinkType() == PTLink::ENROUTE)
		{
			...
			link->mu = ClampNonNegative(static_cast<long double>(link->cost  - cap_cost));
...
void PTNET::IniBoardLinkMultiplier()
{
	...
	for(int i=0;i<numOfLink;i++)
	{
		...
		if (link->GetTransitLinkType() == PTLink::ENROUTE)
		{
			...
			link->volume = tmp;
			link->mu = 0;	
			link->rLink->mu = ClampNonNegative(static_cast<long double>(link->cost  - cap_cost)); //impose such a delay on board links
			if (link->volume>link->cap)
			{
				tn++;
				tvalue += cap_cost/(link->volume - link->cap);
...
floatType PTNET::ComputeBoundGAP()
{
	...
	for(int i=0;i<numOfLink;i++)
	{
		...
		if (link->GetTransitLinkType() == PTLink::ENROUTE &&link->GetTransitLinkCapType()==PTLink::TL)
		{
			...
			gap += ClampNonNegative(static_cast<long double>(pow(ClampNonNegative(static_cast<long double>(link->mu + cb_lambda *(link->volume - link->cap))),2) - pow(link->mu, 2) )); 
		}

		if (link->GetTransitLinkType() == PTLink::ABOARD &&link->GetTransitLinkCapType()==PTLink::BL)
		{
			gap += pow(ClampNonNegative(static_cast<long double>(-cb_lambda*(link->rLink->cap - link->rLink->volume))),2);
		}
	}
...
void PTNET::UpdateMultiplier()
{
	...
	for(int i=0;i<numOfLink;i++)
	{
		...
		if (link->GetTransitLinkType() == PTLink::ENROUTE &&link->GetTransitLinkCapType()==PTLink::TL)
		{
			link->mu = ClampNonNegative(static_cast<long double>(link->mu + cb_lambda *(link->volume - link->cap) ));
		}

		if (link->GetTransitLinkType() == PTLink::ABOARD &&link->GetTransitLinkCapType()==PTLink::BL)
		{
			link->mu = ClampNonNegative(static_cast<long double>(link->mu + cb_lambda *(link->rLink->volume - link->rLink->cap) ));
		}
	}
