...
namespace
{
	string PT_FormatNumber(double value)
	{
		ostringstream oss;
		oss.setf(ios::fixed);
	...
	string PT_FormatInteger(int value)
	{
		ostringstream oss;
		oss<<value;
		return oss.str();
	...
	string PT_PgTextOrNull(const string &value)
	{
		if(value.empty()) return "NULL";
		return TNM_QuotePgLiteral(value);
	}

	string PT_GetNodeStopId(PTNode *node)
	{
		if(node == NULL || node->m_stop == NULL) return "";
		return node->m_stop->m_id;
	}

	string PT_JoinPathLinkIds(TNM_HyperPath *path)
	{
		string text;
		...
		{
			if(i > 0) text += ",";
			text += PT_FormatInteger(glinks[i]->m_linkPtr->id);
		}
		return text;
	}

	string PT_JoinPathLinkProbabilities(TNM_HyperPath *path)
	{
		string text;
		vector<GLINK*> glinks = path->GetGlinks();
		for(size_t i = 0; i < glinks.size(); ++i)
		{
			if(i > 0) text += ",";
			text += PT_FormatNumber(glinks[i]->m_data);
		}
		return text;
	}
}
...
int  PTNET::ReadTEAPODdemand()
{
 ...
 int PTNET::BuildANFromDB(bool stopPos, bool walkfile)
 {
 	TNM_DB_IO_CONFIG dbConfig;
 	string dbErrorMessage;
 	if(!TNM_ParseDbIOConfig(networkName, networkName, dbConfig, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	if(!dbConfig.enabled)
 	{
 		cout<<"\tPostgreSQL input is not enabled"<<endl;
 		return 1;
 	}

 	string stopTable, routeTable, shapeTable, transitTable, tripTable, walkTable;
 	if(!TNM_ResolveTableName(dbConfig.stopTable, dbConfig.defaultSchema, stopTable, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	if(!TNM_ResolveTableName(dbConfig.routeTable, dbConfig.defaultSchema, routeTable, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	if(!TNM_ResolveTableName(dbConfig.shapeTable, dbConfig.defaultSchema, shapeTable, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	if(!TNM_ResolveTableName(dbConfig.transitTable, dbConfig.defaultSchema, transitTable, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	if(!TNM_ResolveTableName(dbConfig.tripTable, dbConfig.defaultSchema, tripTable, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	if(!dbConfig.walkTable.empty())
 	{
 		if(!TNM_ResolveTableName(dbConfig.walkTable, dbConfig.defaultSchema, walkTable, dbErrorMessage))
 		{
 			cout<<dbErrorMessage<<endl;
 			return 1;
 		}
 	...
 	TNM_PGDB db;
 	if(!db.Open(dbConfig.conninfo, dbErrorMessage))
 	{
 		cout<<"Cannot connect to PostgreSQL: "<<dbErrorMessage<<endl;
 		return 1;
 	}

 	vector<vector<string> > rows;
 	cout<<"\tReading the stop table..."<<endl;
 	if(!db.Query("SELECT stop_id, stop_name, x, y FROM " + stopTable + " ORDER BY stop_id", rows, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	...
 	{
 		if(rows[rowIndex].size() < 4)
 		{
 			cout<<"Invalid stop row in PostgreSQL input table."<<endl;
 			return 1;
 		}
 		PTStop *stop = new PTStop;
 		stop->m_id = rows[rowIndex][0];
 		stop->m_name = rows[rowIndex][1];
 		floatType lat = 0.0, lon = 0.0;
 		if(!rows[rowIndex][2].empty() && !rows[rowIndex][3].empty() && TNM_FromString(lat, rows[rowIndex][2], std::dec) && TNM_FromString(lon, rows[rowIndex][3], std::dec))
 		{
 			stop->m_pos.SetPos(lat, lon);
 		}
 		else if(stopPos || !walkfile)
 		{
 			cout<<"Invalid stop coordinates for stop "<<stop->m_id<<endl;
 			delete stop;
 			return 1;
 		}
 		else
 	...
 	rows.clear();
 	cout<<"\tReading the route table..."<<endl;
 	if(!db.Query("SELECT route_id, route_short_name, route_long_name, route_type FROM " + routeTable + " ORDER BY route_id", rows, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	for(size_t rowIndex = 0; rowIndex < rows.size(); ++rowIndex)
 	{
 		if(rows[rowIndex].size() < 4)
 		{
 			cout<<"Invalid route row in PostgreSQL input table."<<endl;
 			return 1;
 		}
 		PTRoute *route = new PTRoute;
 		route->m_id = rows[rowIndex][0];
 		route->m_sname = rows[rowIndex][1];
 		route->m_lname = rows[rowIndex][2];
 		if(!TNM_FromString(route->m_type, rows[rowIndex][3], std::dec))
 		{
 			cout<<"Invalid route type for route "<<route->m_id<<endl;
 			delete route;
 			return 1;
 		}
 	...
 	rows.clear();
 	cout<<"\tReading shape pattern information"<<endl;
 	if(!db.Query("SELECT shape_id, route_id, frequency, capacity, stop_sequence, stop_id FROM " + shapeTable + " ORDER BY shape_id, stop_sequence", rows, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	...
 	for(size_t rowIndex = 0; rowIndex < rows.size(); ++rowIndex)
 	{
 		...
 		{
 			cout<<"Invalid shape row in PostgreSQL input table."<<endl;
 			return 1;
 		}
 		int stopSequence = 0;
 		if(!TNM_FromString(stopSequence, rows[rowIndex][4], std::dec))
 		{
 			cout<<"Invalid stop sequence for shape "<<rows[rowIndex][0]<<endl;
 			return 1;
 		}
 		string shapeid = rows[rowIndex][0];
 		PTShape *shape = GetShapePtr(shapeid);
 		if(shape == NULL)
 		{
 			shape = new PTShape;
 			shape->m_id = shapeid;
 			PTRoute *route = GetRoutePtr(rows[rowIndex][1]);
 			if(!route)
 			{
 				cout<<"\tCannot find route "<<rows[rowIndex][1]<<" for shape "<<shapeid<<endl;
 				delete shape;
 				return 1;
 			}
 			shape->SetRoute(route);
 			double f = 0.0, cap = 0.0;
 			if(!TNM_FromString(f, rows[rowIndex][2], std::dec) || !TNM_FromString(cap, rows[rowIndex][3], std::dec))
 			{
 				cout<<"Invalid frequency or capacity for shape "<<shapeid<<endl;
 				delete shape;
 				return 1;
 			}
 			shape->m_cap = cap;
 			...
 			{
 				delete shape;
 				shape = ret.first->second;
 			}
 		}
 		PTStop *stop = GetStopPtr(rows[rowIndex][5]);
 		if(!stop)
 		{
 			cout<<"\tCannot find stop "<<rows[rowIndex][5]<<" for shape "<<shapeid<<endl;
 			return 1;
 		}
 		shape->AddStop(stop);
 		vector<PTShape*>::iterator ittt = find(stop->mshapes.begin(), stop->mshapes.end(), shape);
 		if(ittt == stop->mshapes.end()) stop->mshapes.push_back(shape);
 	}
 	cout<<"\tRead in total "<<m_shapes.size()<<" shapes, total freqency = "<<ttfreq<<endl;
 	...
 	rows.clear();
 	cout<<"\tReading transit running data from PostgreSQL"<<endl;
 	if(!db.Query("SELECT shape_id, from_stop, to_stop, fft, length FROM " + transitTable + " ORDER BY shape_id, from_stop, to_stop", rows, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	for(size_t rowIndex = 0; rowIndex < rows.size(); ++rowIndex)
 	{
 		...
 		{
 			cout<<"Invalid transit row in PostgreSQL input table."<<endl;
 			return 1;
 		}
 		PTShape* shp = GetShapePtr(rows[rowIndex][0]);
 		if(!shp)
 		{
 			cout<<"shape id "<<rows[rowIndex][0]<<" does not exist."<<endl;
 			return 1;
 		}
 		PTStop* fstop = GetStopPtr(rows[rowIndex][1]);
 		PTStop* tstop = GetStopPtr(rows[rowIndex][2]);
 		if(!fstop || !tstop)
 		{
 			cout<<"stop id "<<rows[rowIndex][1]<<" or "<<rows[rowIndex][2]<<" does not exist."<<endl;
 			return 1;
 		}
 		PTNode* tail = fstop->GetNode(shp->m_id+";ENROUTE");
 		PTNode* head = tstop->GetNode(shp->m_id+";ENROUTE");
 		PTLink *link = CatchLinkPtr(tail, head);
 		if(link == NULL || link->GetTransitLinkType() != PTLink::ENROUTE)
 		{
 			cout<<"link must be enroute, check PostgreSQL transit representation!"<<endl;
 			return 1;
 		}
 		double fft = 0.0, length = 0.0;
 		if(!TNM_FromString(fft, rows[rowIndex][3], std::dec) || !TNM_FromString(length, rows[rowIndex][4], std::dec))
 		{
 			cout<<"Invalid transit fft or length for shape "<<rows[rowIndex][0]<<endl;
 			return 1;
 		}
 		link->pars.push_back(fft);
 		link->length = length;
 		link->fft = fft;
 	}
 	...
 	{
 		rows.clear();
 		if(!db.Query("SELECT from_stop, to_stop, fft, length FROM " + walkTable + " ORDER BY from_stop, to_stop", rows, dbErrorMessage))
 		{
 			cout<<dbErrorMessage<<endl;
 			return 1;
 		}
 		...
 		for(size_t rowIndex = 0; rowIndex < rows.size(); ++rowIndex)
 		{
 			if(rows[rowIndex].size() < 4)
 			{
 				cout<<"Invalid walk row in PostgreSQL input table."<<endl;
 				return 1;
 			}
 			PTStop *fstop = GetStopPtr(rows[rowIndex][0]);
 			PTStop *tstop = GetStopPtr(rows[rowIndex][1]);
 			if(fstop == NULL || tstop == NULL)
 			{
 				cout<<"Walking row references an unknown stop."<<endl;
 				return 1;
 			}
 			double fft = 0.0, length = 0.0;
 			if(!TNM_FromString(fft, rows[rowIndex][2], std::dec) || !TNM_FromString(length, rows[rowIndex][3], std::dec))
 			{
 				cout<<"Invalid walk fft or length from PostgreSQL input table."<<endl;
 				return 1;
 			}
 			lid++;
 			n++;
 	...
 	{
 		cout<<"\tMissing walk_table for PostgreSQL walk input mode"<<endl;
 		return 1;
 	}
 	else if(CreateTEAPWalks(false)!=0)
 	{
 	...
 	rows.clear();
 	cout<<"\tReading the OD demand table..."<<endl;
 	if(!db.Query("SELECT destination_stop, origin_stop, demand FROM " + tripTable + " ORDER BY destination_stop, origin_stop", rows, dbErrorMessage))
 	{
 		cout<<dbErrorMessage<<endl;
 		return 1;
 	}
 	map<string, vector<pair<string, floatType> > > tripMap;
 	for(size_t rowIndex = 0; rowIndex < rows.size(); ++rowIndex)
 	{
 		if(rows[rowIndex].size() < 3)
 		{
 			cout<<"Invalid trip row in PostgreSQL input table."<<endl;
 			return 1;
 		}
 		floatType demand = 0.0;
 		if(!TNM_FromString(demand, rows[rowIndex][2], std::dec))
 		{
 			cout<<"Invalid trip demand for destination "<<rows[rowIndex][0]<<endl;
 			return 1;
 		}
 		tripMap[rows[rowIndex][0]].push_back(make_pair(rows[rowIndex][1], demand));
 	}
 	for(map<string, vector<pair<string, floatType> > >::iterator it = tripMap.begin(); it != tripMap.end(); ++it)
 	{
 		PTStop *destStop = GetStopPtr(it->first);
 		if(destStop == NULL || destStop->GetTransferNode() == NULL)
 		{
 			cout<<"The stop:"<<it->first<<" does not exist!"<<endl;
 			return 1;
 		}
 		PTDestination* pDest = CreatePTDestination(destStop->GetTransferNode()->id, static_cast<int>(it->second.size()));
 		if(pDest == NULL)
 		{
 			cout<<"cannot create static destination object!"<<endl;
 			return 1;
 		}
 		for(size_t originIndex = 0; originIndex < it->second.size(); ++originIndex)
 		{
 			PTStop *orgStop = GetStopPtr(it->second[originIndex].first);
 			if(orgStop == NULL || orgStop->GetTransferNode() == NULL)
 			{
 				cout<<"The stop:"<<it->second[originIndex].first<<" does not exist!"<<endl;
 				return 1;
 			}
 			if(pDest->SetOrg(static_cast<int>(originIndex) + 1, orgStop->GetTransferNode(), it->second[originIndex].second))
 			{
 				pDest->m_tdmd += it->second[originIndex].second;
 				numOfPTTrips += it->second[originIndex].second;
 			}
 			else
 			{
 				cout<<"cannot set org flow:"<<it->second[originIndex].second<<" for dest:"<<it->first<<endl;
 				return 1;
 			}
 		}
 	}
 ...
 int	PTNET::BuildAN(bool stopPos, bool walkfile)
 {
 	...
 	if(dbConfig.enabled && !dbConfig.stopTable.empty())
 	{
 		return BuildANFromDB(stopPos, walkfile);
 	}
...
int PTNET::WritePG()
{
	...
	if(!dbConfig.enabled)
	{
		cout<<"PostgreSQL output is not enabled."<<endl;
		return 1;
	}
	if(dbConfig.linkResultTable.empty()) dbConfig.linkResultTable = "pt_link_result";
	if(dbConfig.pathResultTable.empty()) dbConfig.pathResultTable = "pt_path_result";
	if(dbConfig.iterResultTable.empty()) dbConfig.iterResultTable = "pt_iter_result";
	if(dbConfig.summaryResultTable.empty()) dbConfig.summaryResultTable = "pt_summary_result";

	string linkTable, pathTable, iterTable, summaryTable;
	if(!TNM_ResolveTableName(dbConfig.linkResultTable, dbConfig.defaultSchema, linkTable, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	if(!TNM_ResolveTableName(dbConfig.pathResultTable, dbConfig.defaultSchema, pathTable, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	if(!TNM_ResolveTableName(dbConfig.iterResultTable, dbConfig.defaultSchema, iterTable, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	if(!TNM_ResolveTableName(dbConfig.summaryResultTable, dbConfig.defaultSchema, summaryTable, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	...
	if(!db.Execute("BEGIN", dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }

	if(!db.Execute("DROP TABLE IF EXISTS " + linkTable, dbErrorMessage) ||
		!db.Execute("CREATE TABLE " + linkTable + " (link_id integer, link_type text, tail_node_id integer, head_node_id integer, tail_stop_id text, head_stop_id text, route_id text, shape_id text, seq integer, flow double precision, cost double precision, fcost double precision, capacity double precision)", dbErrorMessage))
	{
		cout<<dbErrorMessage<<endl; return 1;
	}
	for(int i = 0; i < numOfLink; i++)
	{
		PTLink *link = linkVector[i];
		string routeId, shapeId;
		string seqText = "NULL";
		string capText = "NULL";
		if(link->m_shape != NULL)
		{
			shapeId = link->m_shape->m_id;
			if(link->m_shape->m_routePtr != NULL) routeId = link->m_shape->m_routePtr->m_id;
			seqText = PT_FormatInteger(link->seq);
		}
		if(link->GetTransitLinkType() == PTLink::ABOARD && !link->pars.empty())
			capText = PT_FormatNumber(link->pars[link->pars.size() - 1]);
		else if(link->GetTransitLinkType() == PTLink::ENROUTE && link->pars.size() >= 2)
			capText = PT_FormatNumber(link->pars[link->pars.size() - 2]);
		string sql = "INSERT INTO " + linkTable + " (link_id, link_type, tail_node_id, head_node_id, tail_stop_id, head_stop_id, route_id, shape_id, seq, flow, cost, fcost, capacity) VALUES (" +
			PT_FormatInteger(link->id) + "," +
			TNM_QuotePgLiteral(link->GetTransitLinkTypeName()) + "," +
			PT_FormatInteger(link->tail->id) + "," +
			PT_FormatInteger(link->head->id) + "," +
			PT_PgTextOrNull(PT_GetNodeStopId(link->tail)) + "," +
			PT_PgTextOrNull(PT_GetNodeStopId(link->head)) + "," +
			PT_PgTextOrNull(routeId) + "," +
			PT_PgTextOrNull(shapeId) + "," +
			seqText + "," +
			PT_FormatNumber(link->volume) + "," +
			PT_FormatNumber(link->cost) + "," +
			PT_FormatNumber(link->pfdcost) + "," +
			capText + ")";
		if(!db.Execute(sql, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	}

	if(!db.Execute("DROP TABLE IF EXISTS " + pathTable, dbErrorMessage) ||
		!db.Execute("CREATE TABLE " + pathTable + " (origin_node_id integer, destination_node_id integer, origin_stop_id text, destination_stop_id text, path_index integer, path_flow double precision, path_cost double precision, wait_cost double precision, num_links integer, link_sequence text, link_probability text)", dbErrorMessage))
	{
		cout<<dbErrorMessage<<endl; return 1;
	}
	for(int i = 0; i < numOfPTDest; i++)
	{
		...
		{
			PTOrg* org = dest->orgVector[j];
			for(int k = 0; k < (int)org->pathSet.size(); ++k)
			{
				TNM_HyperPath* path = org->pathSet[k];
				vector<GLINK*> glinks = path->GetGlinks();
				string sql = "INSERT INTO " + pathTable + " (origin_node_id, destination_node_id, origin_stop_id, destination_stop_id, path_index, path_flow, path_cost, wait_cost, num_links, link_sequence, link_probability) VALUES (" +
					PT_FormatInteger(org->org->id) + "," +
					PT_FormatInteger(dest->destination->id) + "," +
					PT_PgTextOrNull(PT_GetNodeStopId(org->org)) + "," +
					PT_PgTextOrNull(PT_GetNodeStopId(dest->destination)) + "," +
					PT_FormatInteger(k + 1) + "," +
					PT_FormatNumber(path->flow) + "," +
					PT_FormatNumber(path->cost) + "," +
					PT_FormatNumber(path->WaitCost) + "," +
					PT_FormatInteger(static_cast<int>(glinks.size())) + "," +
					TNM_QuotePgLiteral(PT_JoinPathLinkIds(path)) + "," +
					TNM_QuotePgLiteral(PT_JoinPathLinkProbabilities(path)) + ")";
				if(!db.Execute(sql, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
			}
		}
	}

	if(!db.Execute("DROP TABLE IF EXISTS " + iterTable, dbErrorMessage) ||
		!db.Execute("CREATE TABLE " + iterTable + " (iter integer, conv_rgap double precision, conv_gap double precision, cpu_time double precision, main_loop_time double precision, inner_loop_time double precision, inner_iters integer, num_hyperpaths integer, infeasible_flow double precision, multiplier_conv double precision, lambda double precision)", dbErrorMessage))
	{
		cout<<dbErrorMessage<<endl; return 1;
	}
	for(size_t i = 0; i < iterRecord.size(); i++)
	{
		PTITERELEM* it = iterRecord[i];
		string sql = "INSERT INTO " + iterTable + " (iter, conv_rgap, conv_gap, cpu_time, main_loop_time, inner_loop_time, inner_iters, num_hyperpaths, infeasible_flow, multiplier_conv, lambda) VALUES (" +
			PT_FormatInteger(it->iter) + "," +
			PT_FormatNumber(it->convRGap) + "," +
			PT_FormatNumber(it->convGap) + "," +
			PT_FormatNumber(it->time) + "," +
			PT_FormatNumber(it->mainlooptime) + "," +
			PT_FormatNumber(it->innerlooptime) + "," +
			PT_FormatInteger(it->innerIters) + "," +
			PT_FormatInteger(it->numberofhyperpaths) + "," +
			PT_FormatNumber(it->convIF) + "," +
			PT_FormatNumber(it->multiplierconv) + "," +
			PT_FormatNumber(it->penalty) + ")";
		if(!db.Execute(sql, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	}

	floatType netTTwalkcost = 0.0, netTTinvehcost = 0.0, netTTgcost = 0.0, netTTmu = 0.0, netTTaboardcost = 0.0, netTTalightcost = 0.0;
	for(int i = 0; i < numOfLink; i++)
	{
		PTLink *link = linkVector[i];
		if (PCTAE_ALG==PTNET::PCTAE_algorithm::CAP_PCTAE_P_GP_TL || PCTAE_ALG==PTNET::PCTAE_algorithm::CAP_PCTAE_P_Greedy_TL)
		{
			netTTgcost += link->g_cost * link->volume;
			netTTmu += link->mu * link->volume;
		}
		else
		{
			netTTgcost += link->cost * link->volume;
		}
		switch(link->GetTransitLinkType())
		{
			case PTLink::WALK: netTTwalkcost += link->cost * link->volume; break;
			case PTLink::ENROUTE: netTTinvehcost += link->cost * link->volume; break;
			case PTLink::ABOARD: netTTaboardcost += link->cost * link->volume; break;
			case PTLink::ALIGHT: netTTalightcost += link->cost * link->volume; break;
		}
	}
	floatType netTTpathgcost = 0.0;
	for (int i = 0; i < numOfPTDest; i++)
	...
	double tripDivisor = (numOfPTTrips > 0.0 ? static_cast<double>(numOfPTTrips) : 1.0);

	if(!db.Execute("DROP TABLE IF EXISTS " + summaryTable, dbErrorMessage) ||
		!db.Execute("CREATE TABLE " + summaryTable + " (metric_key text, metric_value double precision)", dbErrorMessage))
	{
		cout<<dbErrorMessage<<endl; return 1;
	}
	vector<pair<string, double> > metrics;
	metrics.push_back(make_pair(string("num_of_nodes"), static_cast<double>(numOfNode)));
	metrics.push_back(make_pair(string("num_of_links"), static_cast<double>(numOfLink)));
	metrics.push_back(make_pair(string("num_of_od_pairs"), static_cast<double>(numOfPTOD)));
	metrics.push_back(make_pair(string("num_of_destinations"), static_cast<double>(numOfPTDest)));
	metrics.push_back(make_pair(string("num_of_trips"), static_cast<double>(numOfPTTrips)));
	metrics.push_back(make_pair(string("avg_net_cost"), static_cast<double>(netTTpathgcost) / tripDivisor));
	metrics.push_back(make_pair(string("avg_mu"), static_cast<double>(netTTmu) / tripDivisor));
	metrics.push_back(make_pair(string("avg_wait_cost"), static_cast<double>(netTTwaitcost) / tripDivisor));
	metrics.push_back(make_pair(string("avg_in_vehicle_cost"), static_cast<double>(netTTinvehcost) / tripDivisor));
	metrics.push_back(make_pair(string("avg_walk_cost"), static_cast<double>(netTTwalkcost) / tripDivisor));
	metrics.push_back(make_pair(string("avg_aboard_cost"), static_cast<double>(netTTaboardcost) / tripDivisor));
	metrics.push_back(make_pair(string("avg_alight_cost"), static_cast<double>(netTTalightcost) / tripDivisor));
	for(size_t m = 0; m < metrics.size(); ++m)
	{
		string sql = "INSERT INTO " + summaryTable + " (metric_key, metric_value) VALUES (" + TNM_QuotePgLiteral(metrics[m].first) + "," + PT_FormatNumber(metrics[m].second) + ")";
		if(!db.Execute(sql, dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	}

	if(!db.Execute("COMMIT", dbErrorMessage)) { cout<<dbErrorMessage<<endl; return 1; }
	return 0;
}
