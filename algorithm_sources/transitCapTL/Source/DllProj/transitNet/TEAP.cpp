...
int	PTNET::PCTAE_Solver(PCTAE_algorithm alg)
{
	...
	switch(alg)
	{
		...
		case PCTAE_algorithm::CAP_PCTAE_IPF_iGP_TL:	 //GP algorithm for suproblem in IPF framework
			return SolveCapacityHyperpathIPF();
		default:
...
bool TNM_HyperPath::InitializeHP(PTNode *org, PTNode *dest)
{
	...
	if(m_links.empty())
	{
		cout<<"\tNo valid hyperpath links were generated for od:"<<org->id<<","<<dest->id<<endl;
		return false;
	}
...
int PTNET::SolvePathTEAP()
{
	...
	UpatePTNetworkLinkCost();
	if(!InitialHyperpathNetFlow())
	{
		cout<<"\tFailed to initialize hyperpath network flow"<<endl;
		return 1;
	}
	UpatePTNetworkLinkCost();
