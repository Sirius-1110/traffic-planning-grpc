...
long long scalar_count(PGconn* connection, const std::string& sql) {
    PGResultPtr result = query_params(connection, sql, {});
...
GeneratedWalkInfo ensure_pt_walk_table(
    const std::string& connectionString,
    const std::string& schema,
    ...
    if (!connection ||
        ...
        throw std::runtime_error(
            "failed to connect database before generating pt_walk: " +
            std::string(
                connection
    ...
    const std::string stopTableName = prefix + kStopInputSuffix;
    const std::string walkTableName = prefix + "pt_walk";
    const std::string stopTable =
        quote_identifier(schema) + "." + quote_identifier(stopTableName);
    const std::string walkTable =
        quote_identifier(schema) + "." + quote_identifier(walkTableName);

    const bool hasStopId =
        column_exists(connection.get(), schema, stopTableName, "stop_id");
    const bool hasNodeId =
        column_exists(connection.get(), schema, stopTableName, "node_id");
    const std::string idColumn =
        hasStopId ? "stop_id" : (hasNodeId ? "node_id" : "");
    if (idColumn.empty()) {
        throw std::runtime_error(
            "failed to generate pt_walk: bus_point needs stop_id or node_id");
    }

    const bool hasGeometry =
        column_exists(connection.get(), schema, stopTableName, "geometry");
    const bool hasX =
        column_exists(connection.get(), schema, stopTableName, "x");
    const bool hasY =
        column_exists(connection.get(), schema, stopTableName, "y");

    GeneratedWalkInfo info;
    if (!hasGeometry && !(hasX && hasY)) {
        if (table_exists(connection.get(), schema, walkTableName) &&
            query_has_rows(
                connection.get(),
                "SELECT 1 FROM " + walkTable + " LIMIT 1")) {
            info.usedExisting = true;
            info.mode = "existing";
            info.rows = scalar_count(
                connection.get(),
                "SELECT COUNT(*) FROM " + walkTable);
            return info;
        }
        throw std::runtime_error(
            "failed to generate pt_walk: bus_point needs geometry or x/y columns");
    }

    std::string geomExpression;
    std::string nonNullPredicate;
    if (hasGeometry) {
        geomExpression = "geometry::geometry";
        nonNullPredicate = "geometry IS NOT NULL";
    } else {
        geomExpression =
            "ST_SetSRID(ST_MakePoint(x::double precision, y::double precision), 4326)";
        nonNullPredicate = "x IS NOT NULL AND y IS NOT NULL";
    }

    execute_command(connection.get(), "BEGIN");
    try {
        execute_command(connection.get(), "DROP TABLE IF EXISTS " + walkTable);
        execute_command(
            connection.get(),
        ...
        execute_command(
            connection.get(),
            "WITH stop_points AS ("
            "  SELECT DISTINCT " + quote_identifier(idColumn) +
            "::text AS stop_id, " + geomExpression + " AS geom "
            "  FROM " + stopTable +
            "  WHERE " + quote_identifier(idColumn) + " IS NOT NULL "
            "    AND btrim(" + quote_identifier(idColumn) + "::text) <> '' "
            "    AND " + nonNullPredicate +
            "), walk_pairs AS ("
            "  SELECT a.stop_id AS from_stop, b.stop_id AS to_stop, "
            "         ST_Distance(a.geom::geography, b.geom::geography) AS length "
            "  FROM stop_points a "
            "  JOIN stop_points b ON a.stop_id < b.stop_id "
            "  WHERE ST_DWithin(a.geom::geography, b.geom::geography, 500.0)"
            ") "
            "INSERT INTO " + walkTable + " (from_stop, to_stop, fft, length) "
            "SELECT from_stop, to_stop, length / 80.0, length "
            "FROM walk_pairs");
        execute_command(
            connection.get(),
            "CREATE INDEX ON " + walkTable + " (from_stop, to_stop)");
        info.generated = true;
        info.mode = "generated";
        info.rows = scalar_count(
            connection.get(),
...
void validate_database_inputs(
    ...
    for (const TableRequirement& requirement : requirements) {
        ...
        if (requirement.suffix == "pt_transit" &&
            actualColumns.find("shape_id") == actualColumns.end() &&
            actualColumns.find("route_id") == actualColumns.end()) {
            throw std::runtime_error(
                "pt_transit 缺少 shape_id 或 route_id 字段；"
                "新版 transit 表应提供 route_id 作为运行序列键");
        }
    ...
    const std::string stop =
        quote_identifier(schema) + "." +
        quote_identifier(prefix + kStopInputSuffix);
    ...
    const std::string way =
        quote_identifier(schema) + "." +
        quote_identifier(prefix + kWayInputSuffix);
    const std::string trip =
        quote_identifier(schema) + "." +
        quote_identifier(prefix + "pt_trip");
    const bool stopHasStopId =
        column_exists(connection.get(), schema, prefix + kStopInputSuffix, "stop_id");
    const bool stopHasNodeId =
        column_exists(connection.get(), schema, prefix + kStopInputSuffix, "node_id");
    const std::string stopIdColumn =
        stopHasStopId ? "stop_id" : (stopHasNodeId ? "node_id" : "");
    if (stopIdColumn.empty()) {
        throw std::runtime_error("bus_point needs stop_id or node_id");
    }
    {
        PGResultPtr stopIdType = query_params(
            connection.get(),
            "SELECT data_type FROM information_schema.columns "
            "WHERE table_schema=$1 AND table_name=$2 AND column_name=$3",
            {schema, prefix + kStopInputSuffix, stopIdColumn});
        if (PQntuples(stopIdType.get()) == 0 ||
            (!type_matches(PQgetvalue(stopIdType.get(), 0, 0), "text") &&
             !type_matches(PQgetvalue(stopIdType.get(), 0, 0), "integer"))) {
            throw std::runtime_error("bus_point id column must be text or integer");
        }
    }
    ...
    const bool transitHasShapeId =
        column_exists(connection.get(), schema, prefix + "pt_transit", "shape_id");
    const bool transitHasRouteId =
        column_exists(connection.get(), schema, prefix + "pt_transit", "route_id");
    const std::string transitKeyColumn =
        transitHasShapeId ? "shape_id" : (transitHasRouteId ? "route_id" : "");

    require_no_rows(
        connection.get(),
        "SELECT 1 FROM " + route +
        " WHERE route_id IS NULL OR btrim(route_id::text)='' LIMIT 1",
        "pt_route.route_id 存在空值");
    require_no_rows(
        connection.get(),
        "SELECT 1 FROM " + stop +
        " WHERE " + quote_identifier(stopIdColumn) +
        " IS NULL OR btrim(" + quote_identifier(stopIdColumn) +
        "::text)='' LIMIT 1",
        "bus_point id column has empty values");
    if (shapeHasShapeId) {
        require_no_rows(
            connection.get(),
            "SELECT 1 FROM " + shape +
            " WHERE shape_id IS NULL OR btrim(shape_id::text)='' "
            "OR route_id IS NULL OR btrim(route_id::text)='' "
            "OR stop_id IS NULL OR btrim(stop_id::text)='' LIMIT 1",
            "pt_shape 的 shape_id、route_id 或 stop_id 存在空值");
    } else {
        std::string shapeNullSql =
            "SELECT 1 FROM " + shape +
            " WHERE route_id IS NULL OR btrim(route_id::text)='' "
            "OR stop_id IS NULL OR btrim(stop_id::text)=''";
        if (shapeHasDir) {
    ...
    require_no_rows(
        ...
        " WHERE " + quote_identifier(transitKeyColumn) +
        " IS NULL OR from_stop IS NULL "
        "OR to_stop IS NULL OR fft IS NULL OR fft<0 LIMIT 1",
        "pt_transit 的 shape_id/route_id、from_stop、to_stop 不能为空，fft 不能为负数");
    if (hasWalkTable) {
        require_no_rows(
            connection.get(),
            "SELECT 1 FROM " + walk +
    ...
    require_no_rows(
        connection.get(),
        "SELECT 1 FROM " + way +
        " WHERE link_id IS NULL OR route_ref IS NULL "
        "OR btrim(route_ref::text)='' OR direction IS NULL "
        "OR btrim(direction::text)='' OR b IS NULL "
        "OR b NOT IN (0,1) OR power IS NULL OR power<=0 "
        "OR init_node IS NULL OR term_node IS NULL LIMIT 1",
        "bus_way 的 link_id、route_ref、direction、b、power、init_node、term_node 不完整；b 必须为 0 或 1，power 必须为正整数");
    require_no_rows(
        connection.get(),
        "SELECT 1 FROM " + trip +
...
void fill_table_names(
    ...
    response.tables["stop_table"] = fullName(kStopInputSuffix);
    ...
    response.tables["way_table"] = fullName(kWayInputSuffix);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        response.attributes["walk_mode"] = "pending_generation";
        response.attributes["algorithm"] = "AON";

        append_progress(args.progressFile, 1, "等待公交分配执行锁...");
        GlobalExecutionLock executionLock;
        append_progress(args.progressFile, 5, "Generating pt_walk from bus_point coordinates...");

        const GeneratedWalkInfo walkInfo =
            ensure_pt_walk_table(connectionString, schema, prefix);
        response.attributes["walk_mode"] = walkInfo.mode;
        response.counts["generated_walk_rows"] = walkInfo.rows;
        response.logs.push_back(
            walkInfo.generated
                ? "pt_walk generated from bus_point coordinates: threshold=500m, speed=80m/min."
                : "pt_walk reused because bus_point has no usable coordinates.");
        append_progress(args.progressFile, 10, "Validating transit input tables...");
