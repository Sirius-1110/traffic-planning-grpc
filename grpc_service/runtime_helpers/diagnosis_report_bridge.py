...
def _resolve_pt_input_suffix(conn, prefix: str, suffix: str) -> str:
    aliases = {
        "bus_point": ("bus_point",),
        "bus_way": ("bus_way",),
    }
    ...
    for candidate in candidates:
        if _table_exists(conn, prefix, candidate):
            return candidate
    return candidates[0]


def _fetch_pipeline_status(conn, prefix: str) -> dict:
    """查询分配/流量表状态，用于报告顶部数据状态说明。"""
    status = {"flow_rows": 0, "road_vol_rows": 0, "assign_error": ""}
    try:
        if _table_exists(conn, prefix, "greedy_link_flow_results"):
            with conn.cursor() as cur:
                cur.execute(f"SELECT COUNT(*) FROM {_tbl(prefix, 'greedy_link_flow_results')}")
                status["flow_rows"] = int(cur.fetchone()[0])
        if _table_exists(conn, prefix, "road_way"):
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT COUNT(*) FROM {_tbl(prefix, 'road_way')} WHERE COALESCE(volume,0)>0"
                )
                status["road_vol_rows"] = int(cur.fetchone()[0])
    except Exception:
        pass
    if status["flow_rows"] <= 0:
        status["assign_error"] = (
            "常见原因：OD 最短路径不可达或路网不连通，请检查质心连杆与节点连通性"
        )
    return status
...
def _metric_num(groups: dict[str, list[dict]], code: str):
    rows = groups.get(code) or []
    if not rows:
        return None
    try:
        p = _pick_primary_value(code, rows)
        return _as_float(p.get("num"))
    except Exception:
        for r in rows:
            n = _as_float(r.get("result_value_num"))
            if n is not None:
                return n
    return None


def _table_columns(conn, prefix: str, suffix: str) -> list[str]:
    if conn is None:
        return []
    suffix = _resolve_pt_input_suffix(conn, prefix, suffix)
    if not _table_exists(conn, prefix, suffix):
        return []
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE table_schema='user_project' AND table_name=%s
...
def _fetch_stop_names(conn, prefix: str) -> dict[str, str]:
    stop_suffix = _resolve_pt_input_suffix(conn, prefix, "bus_point")
    cols = _table_columns(conn, prefix, stop_suffix)
    sid = _first_col(cols, ["node_id"])
    name = _first_col(cols, ["node_name"])
    if not sid or not name:
        return {}
    out = {}
    try:
        with conn.cursor() as cur:
            cur.execute(f"SELECT {_qcol(sid)}, {_qcol(name)} FROM {_tbl(prefix, stop_suffix)}")
            for a, b in cur.fetchall():
                out[str(a)] = str(b or a)
    except Exception:
        return {}
    return out


def _fetch_stop_link_product(conn, prefix: str) -> dict:
    empty = {
        "distribution": [],
        "topSegments": [],
        "avgSaturation": None,
        "maxSaturation": None,
        "highSegmentCount": None,
        "highSegmentRatio": None,
    }
    if conn is None or not _table_exists(conn, prefix, "pt_stop_link_vc_result"):
        return empty
    cols = _table_columns(conn, prefix, "pt_stop_link_vc_result")
    vc_col = _first_col(cols, ["vc_ratio", "v_c", "vc", "saturation"])
    volume_col = _first_col(cols, ["flow", "volume", "passenger_volume", "passenger"])
    from_col = _first_col(cols, ["from_stop", "from_stop_id", "origin_stop"])
    to_col = _first_col(cols, ["to_stop", "to_stop_id", "destination_stop"])
    from_name_col = _first_col(cols, ["from_stop_name", "from_name"])
    to_name_col = _first_col(cols, ["to_stop_name", "to_name"])
    route_count_col = _first_col(cols, ["route_count", "line_count", "routes"])
    if not vc_col:
        return empty
    select_cols = [vc_col]
    for c in (volume_col, from_col, to_col, from_name_col, to_name_col, route_count_col):
        if c and c not in select_cols:
            select_cols.append(c)
    try:
        with conn.cursor() as cur:
            cur.execute(f"SELECT {', '.join(_qcol(c) for c in select_cols)} FROM {_tbl(prefix, 'pt_stop_link_vc_result')}")
            names = [d[0] for d in cur.description]
            rows = [dict(zip(names, r)) for r in cur.fetchall()]
    except Exception:
        return empty
    stop_names = _fetch_stop_names(conn, prefix)
    buckets = [
        ("畅通", 0, 0.8, "#2fa66a"),
        ("接近饱和", 0.8, 1.0, "#91c84c"),
        ("饱和", 1.0, 2.0, "#f2c94c"),
        ("高饱和", 2.0, 5.0, "#f2994a"),
        ("严重饱和", 5.0, None, "#d94b4b"),
    ]
    counts = {b[0]: 0 for b in buckets}
    values = []
    for r in rows:
        vc = _as_float(r.get(vc_col))
        if vc is None:
            continue
        values.append(vc)
        for name, lo, hi, _color in buckets:
            if vc >= lo and (hi is None or vc < hi):
                counts[name] += 1
                break
    total = sum(counts.values())
    distribution = [
        {"name": name, "count": counts[name], "percent": (counts[name] * 100.0 / total if total else 0), "color": color}
        for name, _lo, _hi, color in buckets
    ]

    def segment_name(r):
        fn = str(r.get(from_name_col) or "") if from_name_col else ""
        tn = str(r.get(to_name_col) or "") if to_name_col else ""
        if not fn and from_col:
            fn = stop_names.get(str(r.get(from_col)), str(r.get(from_col) or ""))
        if not tn and to_col:
            tn = stop_names.get(str(r.get(to_col)), str(r.get(to_col) or ""))
        return f"{fn} → {tn}" if fn or tn else "未命名站间"

    top_src = sorted(
        [r for r in rows if _as_float(r.get(vc_col)) is not None],
        key=lambda r: (_as_float(r.get(vc_col)) or 0, _as_float(r.get(volume_col)) or 0),
        reverse=True,
    )[:5]
    top_segments = []
    for i, r in enumerate(top_src, 1):
        vc = _as_float(r.get(vc_col)) or 0
        if vc >= 5:
            advice = "优先复核班次供给与换乘组织"
        elif vc >= 2:
            advice = "建议关注高峰运力匹配"
        elif vc >= 1:
            advice = "关注接近满载时段"
        ...
        top_segments.append({
            ...
            "passengerVolume": _as_float(r.get(volume_col)) if volume_col else None,
            "saturation": vc,
            "routeCount": int(_as_float(r.get(route_count_col)) or 0) if route_count_col else None,
            "advice": advice,
        })
    ...
    return {
        ...
        "avgSaturation": (sum(values) / len(values) if values else None),
        "maxSaturation": (max(values) if values else None),
        "highSegmentCount": high_count if values else None,
        "highSegmentRatio": (high_count * 100.0 / len(values) if values else None),
    }


def _fetch_top_routes_product(conn, prefix: str) -> list[dict]:
    if conn is None:
        return []
    way_suffix = _resolve_pt_input_suffix(conn, prefix, "bus_way")
    if not _table_exists(conn, prefix, way_suffix):
        return []
    cols = _table_columns(conn, prefix, way_suffix)
    route_col = _first_col(cols, ["route_ref", "route_id", "line_id"])
    volume_col = _first_col(cols, ["volume", "passenger_volume"])
    vc_col = _first_col(cols, ["v_c", "vc", "vc_ratio"])
    if not route_col or not volume_col:
        return []
    try:
        avg_part = f", AVG(COALESCE({_qcol(vc_col)},0)) AS avg_vc" if vc_col else ", NULL::double precision AS avg_vc"
        with conn.cursor() as cur:
            cur.execute(
                f"""
                SELECT {_qcol(route_col)} AS route_name,
                       SUM(COALESCE({_qcol(volume_col)},0)) AS total_volume,
                       MAX(COALESCE({_qcol(volume_col)},0)) AS max_section
                       {avg_part}
                FROM {_tbl(prefix, way_suffix)}
                GROUP BY {_qcol(route_col)}
                HAVING SUM(COALESCE({_qcol(volume_col)},0)) > 0
        ...
        return []
    out = []
    for route, total, max_sec, avg_vc in rows:
        out.append({
            "route": str(route or "未命名线路"),
            "intensity": _as_float(total),
            "maxSection": _as_float(max_sec),
            "avgSaturation": _as_float(avg_vc),
            "tip": "重点关注高峰断面",
        })
    return out


def _build_pt_product_data(
    *,
    conn,
    prefix: str,
    groups: dict[str, list[dict]],
    ...
    stop_link = _fetch_stop_link_product(conn, prefix)
    total = _metric_num(groups, "pt_macro_total_demand")
    assigned = _metric_num(groups, "pt_macro_assigned_demand")
    infeasible = _metric_num(groups, "pt_macro_infeasible_flow")
    service_ratio = assigned * 100.0 / total if total and assigned is not None else None
    infeasible_ratio = infeasible * 100.0 / total if total and infeasible is not None else _metric_num(groups, "pt_macro_infeasible_ratio")
    if infeasible_ratio is not None and infeasible_ratio <= 1:
        infeasible_ratio *= 100.0
    metrics = {
        "totalDemand": total,
        "assignedDemand": assigned,
        "serviceRatio": service_ratio,
        "infeasibleDemand": infeasible,
        "infeasibleRatio": infeasible_ratio,
        "routeCount": _metric_num(groups, "pt_meso_total_routes") or _metric_num(groups, "pt_macro_route_count"),
        "activeRoutes": _metric_num(groups, "pt_meso_active_routes"),
        "stopCount": _metric_num(groups, "pt_macro_stop_count"),
        "networkDensity": _metric_num(groups, "pt_macro_bus_density"),
        "populationCoverage500m": _metric_num(groups, "pt_macro_pt_pop_coverage_500m"),
        "areaCoverage": _metric_num(groups, "pt_macro_city_bus_area_coverage"),
        "avgTravelCost": _metric_num(groups, "pt_macro_avg_path_cost"),
        "avgWaitTime": _metric_num(groups, "pt_macro_avg_wait_cost"),
        "waitShare": _metric_num(groups, "pt_macro_wait_cost_share"),
        "avgFrequency": _metric_num(groups, "pt_meso_avg_frequency"),
        "avgSaturation": stop_link.get("avgSaturation") or _metric_num(groups, "pt_macro_avg_stop_link_vc"),
        "maxSaturation": stop_link.get("maxSaturation") or _metric_num(groups, "pt_macro_max_stop_link_vc"),
        "highSegmentCount": stop_link.get("highSegmentCount") or _metric_num(groups, "pt_macro_high_vc_stop_link_count"),
        "highSegmentRatio": stop_link.get("highSegmentRatio") or _metric_num(groups, "pt_macro_high_vc_stop_link_ratio"),
    }
    if metrics["waitShare"] is not None and metrics["waitShare"] <= 1:
        metrics["waitShare"] *= 100.0
    if metrics["highSegmentRatio"] is not None and metrics["highSegmentRatio"] <= 1:
        metrics["highSegmentRatio"] *= 100.0
    pressure = sum(float(x.get("percent") or 0) for x in stop_link.get("distribution", []) if x.get("name") in ("高饱和", "严重饱和"))
    status, focus = "整体运行平稳", "持续监测重点站间客流变化"
    if (metrics.get("infeasibleRatio") or 0) >= 5:
        status, focus = "存在较明显的服务不可达问题", "优先检查不可达 OD、换乘接驳和线路连通"
    elif pressure >= 20 or (metrics.get("maxSaturation") or 0) >= 5:
        status, focus = "局部站间运力压力突出", "优先关注高负荷站间和重点线路运力匹配"
    elif (metrics.get("serviceRatio") or 100) < 95:
        status, focus = "公交可服务需求仍有提升空间", "补强接驳关系和线网覆盖薄弱区域"
    return {
        "projectName": project_name,
        "caseId": cid,
        "baseCaseId": base_cid,
        "reportKind": "基础方案" if report_kind == "base" else "普通方案",
        "generatedAt": ts,
        "summary": {"status": status, "focus": focus},
        "metrics": metrics,
        "saturationDistribution": stop_link.get("distribution") or [],
        "topSegments": stop_link.get("topSegments") or [],
        "topRoutes": _fetch_top_routes_product(conn, prefix),
        "suggestions": [
            {"level": "高优先级", "text": "对高饱和站间进行高峰班次加密、线路截短或运力调配复核。"},
            {"level": "中优先级", "text": "围绕暂不可服务需求检查站点接驳、换乘距离和线路方向连续性。"},
            {"level": "低优先级", "text": "结合 500 米覆盖率和区域覆盖率，识别覆盖空白区并评估补线或站点优化。"},
        ],
    }


PT_PRODUCT_HTML_TEMPLATE = r"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>公交运行诊断报告</title>
__ECHARTS__
<style>
:root{--blue:#123b5d;--cyan:#1489aa;--bg:#f5f8fb;--text:#1f2d3d;--muted:#667085;--line:#e6edf3}
...
.wrap{max-width:1180px;margin:0 auto;padding:28px 24px 42px}.hero{background:linear-gradient(135deg,#0e3a5a,#137f9f);border-radius:22px;color:#fff;padding:28px 34px;box-shadow:0 18px 45px rgba(15,60,90,.18)}
.hero h1{margin:0 0 10px;font-size:30px}.hero p{margin:6px 0;color:rgba(255,255,255,.9);line-height:1.7}.hero .meta{display:flex;gap:14px;flex-wrap:wrap;margin-top:14px;font-size:13px;color:rgba(255,255,255,.76)}
.nav{position:sticky;top:0;z-index:5;margin:18px 0 20px;background:rgba(255,255,255,.92);backdrop-filter:blur(8px);border:1px solid var(--line);border-radius:16px;padding:8px;display:flex;gap:6px;flex-wrap:wrap}.nav a{padding:9px 13px;border-radius:12px;text-decoration:none;color:#31516a;font-size:13px}.nav a:hover{background:#eaf4f8;color:#0e6680}
.section{background:#fff;border:1px solid var(--line);border-radius:20px;margin:18px 0;padding:24px;box-shadow:0 8px 26px rgba(15,45,70,.06)}.section h2{margin:0 0 16px;color:#0f3857;font-size:22px}.lead{margin:0 0 16px;color:#53687a;line-height:1.8}
.cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.card{background:#f8fbfd;border:1px solid #e4eef5;border-radius:16px;padding:16px;min-height:96px;border-left:5px solid var(--cyan)}.card .v{font-size:26px;font-weight:800;color:#103b5a;line-height:1.2}.card .k{font-size:13px;color:#51677a;margin-top:8px}.card .note{font-size:12px;color:#7b8da0;margin-top:8px;line-height:1.5}
.two{display:grid;grid-template-columns:1.15fr .85fr;gap:18px;align-items:start}.three{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.progress{margin:14px 0}.progress .row{display:flex;justify-content:space-between;font-size:13px;color:#486173;margin-bottom:7px}.bar{height:12px;border-radius:99px;background:#e8eff5;overflow:hidden}.bar span{display:block;height:100%;border-radius:99px;background:linear-gradient(90deg,#2fa66a,#1489aa)}
.stack{height:52px;border-radius:12px;overflow:hidden;display:flex;background:#edf2f7;border:1px solid #dde7ef}.stack span{height:100%;min-width:1px}.legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:12px;color:#53687a;font-size:13px}.legend i{display:inline-block;width:10px;height:10px;border-radius:2px;margin-right:5px}.chart{height:300px}
table{width:100%;border-collapse:collapse;table-layout:fixed}th{background:#ecf5fa;color:#1b4a68;text-align:left;font-size:13px;padding:12px}td{border-bottom:1px solid #edf1f5;padding:12px;font-size:13px;line-height:1.55;vertical-align:middle}tr:last-child td{border-bottom:none}.seg{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;word-break:break-word}.advice{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.advice .item{border-radius:16px;padding:16px;background:#f8fbfd;border:1px solid #e4eef5}.advice b{color:#0f3857}
details{margin-top:12px;background:#f8fbfd;border:1px solid #e4eef5;border-radius:14px;padding:14px}summary{cursor:pointer;color:#0f3857;font-weight:700}.empty{color:#7b8da0;background:#f8fbfd;border:1px dashed #ccd9e3;border-radius:14px;padding:16px}footer{color:#8190a0;text-align:center;font-size:12px;margin:26px 0 0}
@media(max-width:900px){.cards,.three,.two,.advice{grid-template-columns:1fr}.wrap{padding:18px 12px}.hero{padding:22px}.chart{height:260px}}
</style></head>
<body><div class="wrap">
<header class="hero"><h1>公交运行诊断报告</h1><p id="hero_text"></p><div class="meta"><span id="meta_project"></span><span id="meta_case"></span><span id="meta_time"></span></div></header>
<nav class="nav"><a href="#overall">总体结论</a><a href="#supply">服务供给与覆盖</a><a href="#experience">可达性与出行体验</a><a href="#pressure">拥挤与运力压力</a><a href="#routes">重点线路</a><a href="#advice">治理建议</a></nav>
<section class="section" id="overall"><h2>总体结论</h2><p class="lead" id="overall_lead"></p><div class="cards" id="overall_cards"></div></section>
<section class="section" id="supply"><h2>服务供给与覆盖</h2><div class="two"><div class="cards" id="supply_cards"></div><div><div class="progress"><div class="row"><span>500 米人口覆盖率</span><b id="pop_cov_text">—</b></div><div class="bar"><span id="pop_cov_bar"></span></div></div><div class="progress"><div class="row"><span>公交区域覆盖率</span><b id="area_cov_text">—</b></div><div class="bar"><span id="area_cov_bar"></span></div></div><p class="lead">覆盖率用于判断公交服务触达范围，数值越高表示站点与线路对人口和空间的覆盖越充分。</p></div></div></section>
<section class="section" id="experience"><h2>可达性与出行体验</h2><div class="cards" id="experience_cards"></div></section>
<section class="section" id="pressure"><h2>拥挤与运力压力</h2><div class="two"><div><div class="stack" id="sat_stack"></div><div class="legend" id="sat_legend"></div><p class="lead" style="margin-top:14px">站间饱和度用于识别运力压力，数值越高表示该站间越需要关注。</p></div><div class="three" id="pressure_cards"></div></div><h3 style="margin:22px 0 10px;color:#0f3857">重点站间</h3><div id="top_segments"></div></section>
<section class="section" id="routes"><h2>重点线路</h2><div id="route_chart" class="chart"></div><div id="route_table"></div></section>
<section class="section" id="advice"><h2>治理建议</h2><div class="advice" id="advice_box"></div><details><summary>方法说明与技术附录</summary><p>本报告基于公交分配结果、站点、线路和站间运行数据生成，用于识别公交服务覆盖、可达性、出行体验和运力压力。</p><p>站间饱和度基于工程容量假设估算，用于识别需要重点关注的站间断面；后续如补充实测满载率或车辆容量，可进一步校准。</p><p>报告仅展示面向业务研判的核心信息，完整计算结果保留在系统结果数据中。</p></details></section>
<footer>公交运行诊断报告 · 仅供交通方案研判参考</footer></div>
<script>
const R=__PT_DATA_JSON__,M=R.metrics||{};function n(v,d=1){v=Number(v);return Number.isFinite(v)?v.toLocaleString('zh-CN',{maximumFractionDigits:d,minimumFractionDigits:0}):'—'}function p(v){v=Number(v);return Number.isFinite(v)?v.toFixed(1)+'%':'—'}function lim(v){v=Number(v);return Number.isFinite(v)?Math.max(0,Math.min(100,v)):0}function card(k,v,note){return '<div class="card"><div class="v">'+v+'</div><div class="k">'+k+'</div>'+(note?'<div class="note">'+note+'</div>':'')+'</div>'}
document.getElementById('hero_text').textContent=(R.summary?.status||'公交运行情况完成诊断')+'，'+(R.summary?.focus||'建议结合重点站间和线路持续优化。');document.getElementById('overall_lead').textContent=document.getElementById('hero_text').textContent;document.getElementById('meta_project').textContent='项目：'+(R.projectName||'—');document.getElementById('meta_case').textContent=R.reportKind+' · case '+(R.caseId??'—');document.getElementById('meta_time').textContent='生成时间：'+(R.generatedAt||'—');
document.getElementById('overall_cards').innerHTML=[card('公交总需求',n(M.totalDemand,0)+' 人次'),card('可服务需求比例',p(M.serviceRatio)),card('暂不可服务需求',n(M.infeasibleDemand,0)+' 人次','占比 '+p(M.infeasibleRatio)),card('高负荷站间占比',p(M.highSegmentRatio))].join('');
document.getElementById('supply_cards').innerHTML=[card('线路数',n(M.routeCount,0)+' 条'),card('站点数',n(M.stopCount,0)+' 个'),card('线网密度',n(M.networkDensity,2)+' km/km²'),card('平均发车频率',n(M.avgFrequency,2))].join('');document.getElementById('pop_cov_text').textContent=p(M.populationCoverage500m);document.getElementById('area_cov_text').textContent=p(M.areaCoverage);document.getElementById('pop_cov_bar').style.width=lim(M.populationCoverage500m)+'%';document.getElementById('area_cov_bar').style.width=lim(M.areaCoverage)+'%';
document.getElementById('experience_cards').innerHTML=[card('可服务需求',n(M.assignedDemand,0)+' 人次'),card('暂不可服务比例',p(M.infeasibleRatio)),card('平均出行时间成本',n(M.avgTravelCost,1)+' 分钟'),card('平均等待时间',n(M.avgWaitTime,1)+' 分钟','等待时间占比 '+p(M.waitShare))].join('');
document.getElementById('pressure_cards').innerHTML=[card('平均站间饱和度',n(M.avgSaturation,2)),card('最高站间饱和度',n(M.maxSaturation,2)),card('高负荷站间数',n(M.highSegmentCount,0)+' 段')].join('');
const dist=R.saturationDistribution||[];document.getElementById('sat_stack').innerHTML=dist.map(x=>'<span title="'+x.name+' '+p(x.percent)+'" style="width:'+lim(x.percent)+'%;background:'+x.color+'"></span>').join('');document.getElementById('sat_legend').innerHTML=dist.map(x=>'<span><i style="background:'+x.color+'"></i>'+x.name+' '+p(x.percent)+'</span>').join('');
const seg=R.topSegments||[];document.getElementById('top_segments').innerHTML=seg.length?'<table><thead><tr><th style="width:56px">排名</th><th>站间</th><th style="width:130px">断面客流</th><th style="width:100px">饱和度</th><th style="width:100px">涉及线路</th><th style="width:210px">建议</th></tr></thead><tbody>'+seg.map(x=>'<tr><td>'+x.rank+'</td><td><div class="seg" title="'+x.segment+'">'+x.segment+'</div></td><td>'+n(x.passengerVolume,1)+'</td><td>'+n(x.saturation,2)+'</td><td>'+(x.routeCount?x.routeCount+' 条':'—')+'</td><td>'+x.advice+'</td></tr>').join('')+'</tbody></table>':'<div class="empty">暂无可展示的重点站间。</div>';
const routes=R.topRoutes||[];if(routes.length&&window.echarts){echarts.init(document.getElementById('route_chart')).setOption({grid:{left:90,right:24,top:10,bottom:30,containLabel:true},tooltip:{trigger:'axis'},xAxis:{type:'value',axisLabel:{formatter:v=>n(v,0)}},yAxis:{type:'category',data:routes.map(x=>x.route).reverse(),axisLabel:{width:110,overflow:'truncate'}},series:[{type:'bar',data:routes.map(x=>x.intensity).reverse(),barMaxWidth:22,itemStyle:{color:'#1489aa',borderRadius:[0,8,8,0]},label:{show:true,position:'right',formatter:a=>n(a.value,0)}}]});document.getElementById('route_table').innerHTML='<table><thead><tr><th>线路</th><th>线路客流强度</th><th>最大断面客流</th><th>诊断提示</th></tr></thead><tbody>'+routes.map(x=>'<tr><td>'+x.route+'</td><td>'+n(x.intensity,1)+'</td><td>'+n(x.maxSection,1)+'</td><td>'+x.tip+'</td></tr>').join('')+'</tbody></table>'}else{document.getElementById('route_chart').outerHTML='<div class="empty">暂无可展示的重点线路。</div>'}
document.getElementById('advice_box').innerHTML=(R.suggestions||[]).map(x=>'<div class="item"><b>'+x.level+'</b><p>'+x.text+'</p></div>').join('');window.addEventListener('resize',()=>document.querySelectorAll('.chart').forEach(el=>{const i=window.echarts&&echarts.getInstanceByDom(el);if(i)i.resize()}));
</script></body></html>
"""


def _render_pt_product_html(
    *,
    report_kind: str,
    pid: int,
    uid: int,
    ...
    html = PT_PRODUCT_HTML_TEMPLATE.replace("__ECHARTS__", _load_echarts_script())
    html = html.replace("__PT_DATA_JSON__", json.dumps(data, ensure_ascii=False))
    return html, {"missing_notes": [], "report_warnings": []}
...
def _render_html(
    *,
    ...
    prefix = _prefix(pid, uid, cid)
    if type_filter == "pt":
        return _render_pt_product_html(
            report_kind=report_kind,
            pid=pid,
            uid=uid,
            cid=cid,
...
def main() -> None:
    ...
    try:
        ...
        pipeline = _fetch_pipeline_status(conn, prefix)
        logs.append(
