...
INDICATOR_LABELS = {
    ...
    "slow_macro_walk_continuity": "步行网络连续度",
    "slow_macro_bike_continuity": "骑行网络连续度",
...
def _pick_card_primary_value(code: str, rows: list[dict]) -> dict:
    ...
    if code == "slow_macro_vc_distribution":
        severe = [
            r for r in rows
            if int(r.get("sort_order") or 0) == 4
            and r.get("result_value_num") is not None
        ]
        if severe:
            v = float(severe[0].get("result_value_num") or 0)
            return {"label": "严重拥堵占比", "num": None, "text": f"严重拥堵 {v:.1f}%", "unit": ""}
    ...
    if code in {"slow_micro_vc", "slow_micro_topn_vc"}:
        vc_rows = [
            r for r in rows
            if (r.get("result_item") or "").lower() == "v_c"
            and r.get("result_value_num") is not None
        ]
        if vc_rows:
...
def _build_narratives(
    ...
    if flow_rows <= 0 or vol_rows <= 0:
        exec_lines.append(
            ...
            f"（greedy_link_flow_results={flow_rows} 行，road_way.volume&gt;0 共 {vol_rows} 条）。"
...
def _segment_bar_data(rows: list[dict], limit: int = 25) -> dict | None:
    """中微观指标 → Top-N 柱状图数据。"""
    ...
    for r in rows:
        ...
        if ctx.upper() == "GLOBAL" and len(rows) > 3:
            continue
        candidates.append((_segment_label(r), float(r["result_value_num"]), (r.get("result_unit") or "").strip()))
    if not candidates:
        for r in _global_rows(rows):
            if r.get("result_value_num") is not None:
                candidates.append(
                    (_segment_label(r), float(r["result_value_num"]), (r.get("result_unit") or "").strip())
                )
    if not candidates:
        return None
...
def _build_indicator_cards(groups: dict[str, list[dict]], *, scope: str) -> list[dict]:
    ...
    for code in sorted(groups.keys()):
        ...
        if code == "slow_macro_vc_distribution":
            ...
            if severe_rows:
                card["value"] = f"严重拥堵 {float(severe_rows[0].get('result_value_num') or 0):.1f}%"
            card["unit"] = ""
...
def _build_chart_payload(
    ...
    *,
    ...
    indicator_cards = _build_indicator_cards(groups, scope=scope)
...
def _build_module_count_summary(counts: dict, *, scope: str = _SCOPE_MACRO) -> str:
    ...
    lines = [
        line("机动车", road_m, road_mm),
        line("慢行", slow_m, slow_mm),
        line("公共交通", pt_m, pt_mm),
    ]
...
def _ai_dialog_html(param2: dict, report_kind: str, pid: int, uid: int, cid: int, logs: list[str] | None = None) -> str:
    ...
    try:
        ...
        if not md:
            ...
            return ""
        parts = _split_ai_md(md)
        blocks = []
        if parts.get("diagnosis"):
            blocks.append(
                '<div id="s_ai_diag" class="section ai-section">'
                '<div class="sec-title"><span class="dot dm"></span>AI 智能诊断（对话输出）</div>'
                '<div class="ai-card ai-card-red">' + _ai_md_to_html(parts["diagnosis"]) + "</div></div>"
            )
        if parts.get("advice"):
            blocks.append(
                '<div id="s_ai_advice" class="section ai-section">'
                '<div class="sec-title"><span class="dot ds"></span>AI 建议（对话输出）</div>'
                '<div class="ai-card ai-card-green">' + _ai_md_to_html(parts["advice"]) + "</div></div>"
            )
        if not blocks:
            blocks.append(
                '<div id="s_ai_all" class="section ai-section">'
                '<div class="sec-title"><span class="dot dr"></span>AI 对话记录</div>'
                '<div class="ai-card">' + _ai_md_to_html(md) + "</div></div>"
            )
        if logs is not None:
...
<div class="banner">
  ...
  <div class="meta">项目名 __PROJECT_NAME__ · 项目 ID __PID__ · 用户 __UID__ · 方案 case__CID__<br>表前缀 __PREFIX__<br>生成时间 __TS__</div>
</div>
...
<div class="container">
  ...
  <div id="s4" class="section" style="display:none">
    ...
    <div class="g2" id="s4_charts">
      ...
      <div class="chart-slot card" id="slot_c_compare_delta"><h3>指标变化量（本方案 − 基础方案）</h3>
        <div id="c_compare_delta" style="height:320px"></div></div>
    </div>
  </div>
  <div id="s6" class="section">
    <div class="sec-title"><span class="dot dr"></span>中微观指标分段图表</div>
    <div class="desc">按路段、线路或空间单元展示中微观诊断指标 Top-N 分布（有数值才渲染图表）。</div>
...
function hideSection(secId,navHref){
  const s=AV.sections[secId]; if(!s||s.ok) return;
  hideNav(navHref);
...
[{h:'#s0',t:'总览'},{h:'#s_maps',t:'空间分布'},{h:'#s1',t:'机动车'},{h:'#s2',t:'慢行'},{h:'#s3',t:'公交'}]
...
['c_road_cong','c_road_sat','c_road_bar','c_road_cap','c_road_eff','c_slow_den','c_slow_vc','c_pt_bar','c_compare_bar','c_compare_delta']
...
['road','slow','pt'].forEach(mod=>{
  ...
  cards.forEach(c=>{
    ...
    d.innerHTML='<div class="it">'+c.tier_label+' · '+c.label+'</div>'
      +'<div class="iv">'+c.value+'<span class="iu">'+((c.unit&&c.value!=='—')?c.unit:'')+'</span></div>'
...
(D.charts.kpis||[]).forEach(k=>{
  ...
  d.innerHTML='<div class="kv">'+k.value+'<span class="ku">'+(k.unit||'')+'</span></div><div class="kl">'+k.label+'</div>';
  kr.appendChild(d);
});
...
if(chartOk('c_slow_den')){
  ec('c_slow_den').setOption({
    tooltip:{trigger:'axis'},xAxis:{type:'category',data:sd.map(x=>x.name)},
    yAxis:{type:'value',name:'km/km²'},series:[{type:'bar',data:sd.map(x=>x.value),
      itemStyle:{color:'#27ae60'},barMaxWidth:40,label:{show:true,position:'top'}}]
  });
}
const vc=D.charts.vc_pie||[];
if(chartOk('c_slow_vc')){
  ec('c_slow_vc').setOption({
    tooltip:{trigger:'item',formatter:'{b}: {c}%'},
    series:[{type:'pie',radius:'65%',data:vc,label:{formatter:'{b}\n{d}%'}}]
  });
}
...
if(chartOk('c_compare_delta')){
  const deltas=(D.charts.compare||[]).filter(x=>x.delta!=null);
  if(deltas.length){
    ec('c_compare_delta').setOption({
      tooltip:{trigger:'axis'},grid:{left:10,right:30,top:10,bottom:10,containLabel:true},
      xAxis:{type:'value'},
      yAxis:{type:'category',data:deltas.map(x=>x.name),axisLabel:{fontSize:9}},
      series:[{type:'bar',data:deltas.map(x=>({value:x.delta,
        itemStyle:{color:x.delta>0?'#e74c3c':'#27ae60'}})),label:{show:true,position:'right',fontSize:9}}]
    });
  }
}
...
(D.charts.meso_charts||[]).forEach(ch=>{
  ...
  if(!inst) return;
  inst.setOption({
    tooltip:{trigger:'axis'},grid:{left:8,right:16,top:8,bottom:60,containLabel:true},
    xAxis:{type:'category',data:ch.labels,axisLabel:{rotate:35,fontSize:9,interval:0}},
    yAxis:{type:'value',name:ch.unit||''},
    series:[{type:'bar',barMaxWidth:22,data:ch.values,
      itemStyle:{color:modColor[ch.module]||'#2d6a9f'},
      label:{show:ch.values.length<=8,position:'top',fontSize:8}}]
  });
});

const tb=document.getElementById('detail_body');
const metaMap=D.charts.indicator_meta||{};
(D.charts.detail||[]).forEach(r=>{
  const m=metaMap[r.code]||{};
  const tr=document.createElement('tr');
  tr.innerHTML='<td>'+(modName[r.module]||r.module)+'</td><td>'+(r.tier||'')+'</td><td>'+r.label+'</td>'
    +'<td style="font-size:11px;max-width:220px">'+(m.formula||'—')+'</td>'
    +'<td style="font-size:11px;color:#666">'+(m.reference||'—')+'</td>'
...
def _compare_global(base_groups: dict, scheme_groups: dict) -> list[dict]:
    """改扩建对比：双方均有的宏观指标，且均有可对比代表值。"""
    rows = []
    for code in sorted(set(base_groups) & set(scheme_groups)):
        if indicator_tier(code) != "macro":
            continue
        b = _pick_primary_value(code, base_groups[code]).get("num")
        s = _pick_primary_value(code, scheme_groups[code]).get("num")
...
def _render_html(
    *,
    ...
    title = "基础方案诊断指标报告" if report_kind == "base" else "方案诊断指标报告"
    scope_note = {
        _SCOPE_MACRO: "宏观诊断指标",
        _SCOPE_MESO: "中微观诊断指标",
        "all": "宏观+中微观诊断指标",
    }.get(scope, "宏观诊断指标")
    subtitle = f"{scope_note}可视化分析报告 · 维度 {type_filter} · 方案前缀 {prefix}"
    ...
    charts = _build_chart_payload(groups, compare_rows, scope=scope)
    ...
    data = {
        ...
        "scope": scope,
        "module_count_summary": _build_module_count_summary(charts.get("counts") or {}, scope=scope),
    }
        ...
        try:
            if report_kind == "scheme" and base_cid is not None:
                base_bundle, base_meta = collect_map_bundle_for_cases(
                    conn, pid, uid, [base_cid, 3], map_logs or []
                )
                scheme_bundle, scheme_meta = collect_map_bundle_for_cases(
                    conn, pid, uid, [cid, 3], map_logs or []
                )
                compare_layers = [
                    {
                        "key": "base",
                ...
                cur_bundle, cur_meta = collect_map_bundle_for_cases(
                    conn, pid, uid, [cid, ref_cid], map_logs or []
                )
                ref_bundle, ref_meta = collect_map_bundle_for_cases(
                    conn, pid, uid, [ref_cid, cid], map_logs or []
                )
                compare_layers = [
                    {
                        "key": "current",
...
def main() -> None:
    ...
    try:
        ...
        if kind == "scheme" and base_cid is not None:
            ...
            compare_rows = _compare_global(group_by_indicator(base_rows), groups)
