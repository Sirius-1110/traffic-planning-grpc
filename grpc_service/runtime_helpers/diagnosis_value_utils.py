...
PRIMARY_RESULT_KEY: dict[str, list[str]] = {
    ...
    "pt_macro_pt_pop_coverage_500m": ["overall"],
    "pt_macro_city_bus_area_coverage": ["overall"],
    "pt_macro_rail_pop_coverage_800m": ["overall"],
}
...
PRIMARY_RESULT_ITEM: dict[str, list[str]] = {
    ...
    "pt_macro_pt_pop_coverage_500m": ["assigned_ratio", "infeasible_ratio"],
    "pt_macro_rail_pop_coverage_800m": ["cpu_time", "num_hyperpaths"],
    "pt_macro_bus_density": ["num_of_routes", "num_of_stops"],
    "pt_macro_city_bus_area_coverage": ["positive_flow_ratio", "avg_path_cost"],
}

MAX_PLAUSIBLE_SPEED_KMH = 180.0
MAX_PLAUSIBLE_DENSITY_KM2 = 500.0
MAX_PLAUSIBLE_PT_BUS_DENSITY = 1_000_000.0
CONTINUITY_KM_CODES = frozenset({
...
_SKIP_RESULT_ITEMS: dict[str, frozenset[str]] = {
    ...
    "pt_macro_city_bus_area_coverage": frozenset({"total_area_km2", "total_area", "covered_area"}),
...
def is_plausible_value(code: str, num: float | None, unit: str = "") -> bool:
    ...
    if code == "pt_macro_bus_density":
        return 0 <= v <= MAX_PLAUSIBLE_PT_BUS_DENSITY
    u = (unit or "").strip()
    if code in ("pt_macro_pt_pop_coverage_500m", "pt_macro_city_bus_area_coverage"):
        if "%" in u:
            return 0 <= v <= 100.0
...
def pick_primary_value(code: str, rows: list[dict], *, label: str | None = None) -> dict[str, Any]:
    ...
    def pack(r: dict, key: str, num: float) -> dict[str, Any]:
        ...
        num = normalize_length_km_display(code, num)
        return {
            "code": code,
            "label": lbl,
            "key": key,
...
def normalize_length_km_display(code: str, num: float | None) -> float | None:
    """连续度等指标若误存为米，展示前换算为 km。"""
    if num is None or code not in CONTINUITY_KM_CODES:
        return num
    v = float(num)
    if v > 500.0:
...
# 入库时 overall 与「全网统归」等别名会双写，报告只保留优先 key 的一组
_RESULT_KEY_ALIASES: dict[str, list[str]] = {
    "overall": ["overall", "全网统归", "全网汇总"],
}


def preferred_macro_rows(rows: list[dict], code: str) -> list[dict]:
    """宏观展示用行：优先 overall 等主键，避免 overall/全网统归 重复。"""
    g = global_rows(rows)
    if not g:
        g = list(rows)
    prefs = PRIMARY_RESULT_KEY.get(code) or ["overall"]
    alias_groups = [_RESULT_KEY_ALIASES.get(p, [p]) for p in prefs]
    for aliases in alias_groups:
        for rk in aliases:
            subset = [r for r in g if (r.get("result_key") or "") == rk]
            if subset:
                return subset
    seen: set[str] = set()
    out: list[dict] = []
    for r in g:
        item = row_item(r) or (r.get("result_key") or "")
        if item in seen:
            continue
        seen.add(item)
        out.append(r)
